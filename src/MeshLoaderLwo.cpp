...
lwClip *lwGetClip( idFile *fp, int cksize )
{
   ...
   clip = (lwClip*)mem_cleared_alloc( sizeof( lwClip ) );
   if ( !clip ) goto Fail;
   ...
   while ( 1 ) {
      ...
      switch ( id ) {
         ...
         case ID_IFLT:
         case ID_PFLT:
            filt = (lwPlugin*)mem_cleared_alloc( sizeof( lwPlugin ) );
            if ( !filt ) goto Fail;
...
lwEnvelope *lwGetEnvelope( idFile *fp, int cksize )
{
   ...
   env = (lwEnvelope*)mem_cleared_alloc( sizeof( lwEnvelope ) );
   if ( !env ) goto Fail;
   ...
   while ( 1 ) {
      ...
      switch ( id ) {
         ...
         case ID_KEY:
            key = (lwKey*)mem_cleared_alloc( sizeof( lwKey ) );
            if ( !key ) goto Fail;
            key->time = getF4( fp );
            key->value = getF4( fp );
         ...
         case ID_CHAN:
            plug = (lwPlugin*)mem_cleared_alloc( sizeof( lwPlugin ) );
            if ( !plug ) goto Fail;
...
lwObject *lwGetObject( const char *filename, unsigned int *failID, int *failpos )
{
   ...
   object = (lwObject*)mem_cleared_alloc( sizeof( lwObject ) );
   if ( !object ) goto Fail;

   layer = (lwLayer*)mem_cleared_alloc( sizeof( lwLayer ) );
   if ( !layer ) goto Fail;
   object->layer = layer;
...
static int add_tvel( float pos[], float vel[], lwEnvelope **elist, int *nenvs )
{
   ...
   for ( i = 0; i < 3; i++ ) {
      env = (lwEnvelope*)mem_cleared_alloc( sizeof( lwEnvelope ) );
      key0 = (lwKey*)mem_cleared_alloc( sizeof( lwKey ) );
      key1 = (lwKey*)mem_cleared_alloc( sizeof( lwKey ) );
      ...
      env->index = *nenvs + i + 1;
      env->type = 0x0301 + i;
      env->name = (char*)mem_cleared_alloc( 11 );
      if ( env->name ) {
...
static lwTexture *get_texture( char *s )
{
   ...
   tex = (lwTexture*)mem_cleared_alloc( sizeof( lwTexture ) );
   if ( !tex ) return NULL;
...
lwSurface *lwGetSurface5( idFile *fp, int cksize, lwObject *obj )
{
   ...
   surf = (lwSurface*)mem_cleared_alloc( sizeof( lwSurface ) );
   if ( !surf ) goto Fail;
   ...
   while ( 1 ) {
      ...
      switch ( id ) {
         ...
         case ID_SHDR:
            shdr = (lwPlugin*)mem_cleared_alloc( sizeof( lwPlugin ) );
            if ( !shdr ) goto Fail;
...
int lwGetPointPolygons( lwPointList *point, lwPolygonList *polygon )
{
   ...
   for ( i = 0; i < point->count; i++ ) {
      ...
      point->pt[ i ].pol = (int*)mem_cleared_alloc( point->pt[ i ].npols * sizeof( int ) );
   ...
   lwSurface **surf, int *nsurfs )
{
   ...
   s = (lwSurface**)mem_cleared_alloc( tlist->count * sizeof( lwSurface * ) );
   if ( !s ) return 0;
   ...
   for ( i = 0; i < polygon->count; i++ ) {
      ...
      if ( !s[ index ] ) {
         ...
         if ( !s[ index ] ) return 0;
         s[ index ]->name = (char*)mem_cleared_alloc( strlen( tlist->tag[ index ] ) + 1 );
   ...
   while ( 1 ) {
      ...
      switch ( id ) {
         ...
         case ID_FKEY:
            ...
            tex->param.grad.key = (lwGradKey*)mem_cleared_alloc( nkeys * sizeof( lwGradKey ) );
         ...
         case ID_IKEY:
            nkeys = sz / 2;
            tex->param.grad.ikey = (short*)mem_cleared_alloc( nkeys * sizeof( short ) );
   ...
   int perpoly )
{
   ...
   vmap = (lwVMap*)mem_cleared_alloc( sizeof( lwVMap ) );
   if ( !vmap ) {
      mem_free( buf );
      return NULL;
   }
   ...
   vmap->nverts = npts;
   vmap->vindex = (int*)mem_cleared_alloc( npts * sizeof( int ) );
   if ( !vmap->vindex ) goto Fail;
   if ( perpoly ) {
      vmap->pindex = (int*)mem_cleared_alloc( npts * sizeof( int ) );
      if ( !vmap->pindex ) goto Fail;
   }

   if ( vmap->dim > 0 ) {
      vmap->val = (float**)mem_cleared_alloc( npts * sizeof( float * ) );
      if ( !vmap->val ) goto Fail;
      f = (float*)mem_cleared_alloc( npts * vmap->dim * sizeof( float ) );
      if ( !f ) goto Fail;
      for ( i = 0; i < npts; i++ )
...
int lwGetPointVMaps( lwPointList *point, lwVMap *vmap )
{
   ...
   for ( i = 0; i < point->count; i++ ) {
      if ( point->pt[ i ].nvmaps ) {
         point->pt[ i ].vm = (lwVMapPt*)mem_cleared_alloc( point->pt[ i ].nvmaps * sizeof( lwVMapPt ) );
...
int lwGetPolyVMaps( lwPolygonList *polygon, lwVMap *vmap )
{
   ...
   for ( i = 0; i < polygon->count; i++ ) {
      for ( j = 0; j < polygon->pol[ i ].nverts; j++ ) {
         ...
         if ( pv->nvmaps ) {
            pv->vm = (lwVMapPt*)mem_cleared_alloc( pv->nvmaps * sizeof( lwVMapPt ) );
            if ( !pv->vm ) return 0;
            pv->nvmaps = 0;
         }
      }
