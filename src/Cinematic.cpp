...
private:
	...
	void					RoQ_init( void );
	void					blitVQQuad32fs( unsigned char **status, unsigned char *data );
	void					RoQShutdown( void );
	void					RoQInterrupt(void);

	void					move8_32( unsigned char *src, byte *dst, int spl );
	void					move4_32( unsigned char *src, byte *dst, int spl );
	void					blit8_32( unsigned char *src, byte *dst, int spl );
	void					blit4_32( unsigned char *src, byte *dst, int spl );
	void					blit2_32( unsigned char *src, byte *dst, int spl );

	unsigned short			yuv_to_rgb( long y, long u, long v );
	unsigned int			yuv_to_rgb24( long y, long u, long v );

	void					decodeCodeBook( unsigned char *input, unsigned short roq_flags );
	void					recurseQuad( long startX, long startY, long quadSize, long xOff, long yOff );
	void					setupQuad( long xOff, long yOff );
	void					readQuadInfo( unsigned char *qData );
...
void idCinematic::InitCinematic( void ) {
	...
	vq2 = (u16*)mem_alloc( 256*16*4 * sizeof( unsigned short ) );
	vq4 = (u16*)mem_alloc( 256*64*4 * sizeof( unsigned short ) );
	vq8 = (u16*)mem_alloc( 256*256*4 * sizeof( unsigned short ) );
}
...
idCinematicLocal::idCinematicLocal() {
	...
	qStatus[0] = (unsigned char **)mem_alloc( 32768 * sizeof( byte *) );
	qStatus[1] = (unsigned char **)mem_alloc( 32768 * sizeof( byte *) );
}
...
void idCinematicLocal::move8_32( unsigned char *src, byte *dst, int spl ) {
#if 1
	int *dsrc, *ddst;
	int dspl;
...
==============
*/
void idCinematicLocal::move4_32( unsigned char *src, byte *dst, int spl  ) {
#if 1
	int *dsrc, *ddst;
	int dspl;
...
==============
*/
void idCinematicLocal::blit8_32( unsigned char *src, byte *dst, int spl  ) {
#if 1
	int *dsrc, *ddst;
	int dspl;
...
==============
*/
void idCinematicLocal::blit4_32( unsigned char *src, byte *dst, int spl  ) {
#if 1
	int *dsrc, *ddst;
	int dspl;
...
==============
*/
void idCinematicLocal::blit2_32( unsigned char *src, byte *dst, int spl  ) {
#if 1
	int *dsrc, *ddst;
	int dspl;
...
==============
*/
void idCinematicLocal::blitVQQuad32fs( unsigned char **status, unsigned char *data ) {
	...
	do {
		...
		switch (code) {
			case	0x8000:													// vq code
				blit8_32( (unsigned char *)&vq8[(*data)*128], status[index], samplesPerLine );
				data++;
				index += 5;
				break;
			case	0xc000:													// drop
				...
				for(i=0;i<4;i++) {
					...
					switch (code) {											// code in top two bits of code
						case	0x8000:										// 4x4 vq code
							blit4_32( (unsigned char *)&vq4[(*data)*32], status[index], samplesPerLine );
							data++;
							break;
						case	0xc000:										// 2x2 vq code
							blit2_32( (unsigned char *)&vq2[(*data)*8], status[index], samplesPerLine );
							data++;
							blit2_32( (unsigned char *)&vq2[(*data)*8], status[index]+8, samplesPerLine );
							data++;
							blit2_32( (unsigned char *)&vq2[(*data)*8], status[index]+samplesPerLine*2, samplesPerLine );
							data++;
							blit2_32( (unsigned char *)&vq2[(*data)*8], status[index]+samplesPerLine*2+8, samplesPerLine );
							data++;
							break;
...
==============
*/
void idCinematicLocal::decodeCodeBook( unsigned char *input, unsigned short roq_flags ) {
	long	i, j, two, four;
...
==============
*/
void idCinematicLocal::readQuadInfo( unsigned char *qData ) {
	...
	if (!image ) {
		image = (unsigned char *)mem_alloc( CIN_WIDTH*CIN_HEIGHT*samplesPerPixel*2 );
	}
 ...
 * (ignoring the current state of next_input_unsigned char & bytes_in_buffer),
 * reset the pointer & count to the start of the buffer, and return TRUE
 * indicating that the buffer has been reloaded.  It is not necessary to
 * fill the buffer entirely, only to obtain at least one more unsigned char.
 *
 ...
 * (typically the start of the current MCU). next_input_unsigned char & bytes_in_buffer
 ...
 * Arranging for additional unsigned chars to be discarded before reloading the input
 * buffer is the application writer's problem.
 */

METHODDEF void
skip_input_data (j_decompress_ptr cinfo, long num_unsigned chars)
{
  my_src_ptr src = (my_src_ptr) cinfo->src;
   ...
   */
  if (num_unsigned chars > 0) {
	src->infile = src->infile + num_unsigned chars;
    src->pub.next_input_unsigned char += (size_t) num_bytes;
    src->pub.unsigned chars_in_buffer -= (size_t) num_bytes;
  }
}
...
GLOBAL void
jpeg_memory_src (j_decompress_ptr cinfo, unsigned char *infile, int size)
{
  my_src_ptr src;
  ...
  src->infile = infile;
  src->memsize = size;
  src->pub.unsigned chars_in_buffer = 0; /* forces fill_input_buffer on first read */
  src->pub.next_input_unsigned char = NULL; /* until buffer loaded */
}

int JPEGBlit( unsigned char *wStatus, byte *data, int datasize )
{
...
//	cinfo.out_color_space = JCS_GRAYSCALE;
  ...
  while (cinfo.output_scanline < cinfo.output_height) {
	...
	for(x=0;x<cinfo.output_width;x++) {
		...
		unsigned char *roof = (byte *)&pixel;
		unsigned char temp = roof[0];
		roof[0] = roof[2];
		roof[2] = temp;
		out[x] = pixel;
	}
